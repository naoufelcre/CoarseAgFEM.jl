module MeshCoarsening

using Gridap
using Gridap.Geometry
using Gridap.Arrays
using ..QuadtreeMeshing
using ..GridapIntegration: quadtree_to_discrete_model
using ..QuadtreeMeshing: QuadtreeTransfer

export initialize_builder, build_coarse_model

"""
    initialize_builder(model::CartesianDiscreteModel)
...
"""
function initialize_builder(model::CartesianDiscreteModel)
    ...
    if isempty(roots)
         error("[MeshCoarsening] No nodes created!")
    end
...
function build_coarse_model(fine_model::CartesianDiscreteModel, level_set_func::Function; 
    ...
    builder = initialize_builder(fine_model)
    ...
    op = QuadtreeTransfer(raw_map, (nx, ny), length(elements), builder.roots, leaf_to_cell_ids)
